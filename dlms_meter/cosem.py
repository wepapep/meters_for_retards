...
# ===========================================================================
# Conformance bitstring decoder
# ===========================================================================

# DLMS conformance bits (Green Book, COSEM Application Layer)
# The wire format is "5F 04 <unused_bits> <byte0> <byte1> <byte2>"
# where bits are read MSB-first across the 3 data bytes.
_CONFORMANCE_BITS = {
    3:  "read",
    4:  "write",
    5:  "unconfirmed_write",
    8:  "attribute0_supported_with_set",
    9:  "priority_mgmt_supported",
    10: "attribute0_supported_with_get",
    11: "block_transfer_with_get_or_read",
    12: "block_transfer_with_set_or_write",
    13: "block_transfer_with_action",
    14: "multiple_references",
    15: "information_report",
    16: "data_notification",
    17: "access",
    18: "parameterized_access",
    19: "get",
    20: "set",
    21: "selective_access",
    22: "event_notification",
    23: "action",
}
...
class Capabilities:
    """
    Decoded DLMS conformance bits — which services the meter supports.

    LN-style services (modern meters):
        get, set, action, event_notification

    SN-style services (older meters incl. L+G ZMD):
        read, write, unconfirmed_write

    Use supports("get") for explicit checks, or the convenience properties:
        can_read_ln, can_read_sn, can_write_ln, can_write_sn
    """
    raw_conformance: bytes
    supported:       set[str]

    def supports(self, service: str) -> bool:
        """Check if a named service is supported."""
        return service in self.supported

    @property
    def can_read_ln(self) -> bool:
        """Can do GET.request (LN referencing)."""
        return "get" in self.supported

    @property
    def can_read_sn(self) -> bool:
        """Can do READ.request (SN referencing)."""
        return "read" in self.supported

    @property
    def can_write_ln(self) -> bool:
        return "set" in self.supported

    @property
    def can_write_sn(self) -> bool:
        return "write" in self.supported

    def __str__(self) -> str:
        services = ", ".join(sorted(self.supported)) or "none"
        return f"Capabilities({services})"


def decode_conformance(data: bytes) -> Capabilities:
    """
    Decode a conformance bitstring into a Capabilities object.

    Args:
        data: the 6-byte conformance TLV "5F 04 <unused> <3 bytes>"
              (as returned in AssociationResult.negotiated_conformance)
              or the 4-byte payload "<unused> <3 bytes>" (without the 5F 04 tag).
    """
    if len(data) == 6 and data[0] == 0x5F and data[1] == 0x04:
        payload = data[2:]
    elif len(data) == 4:
        payload = data
    else:
        raise ValueError(f"Unexpected conformance length: {len(data)} bytes")

    # payload[0] = number of unused trailing bits (typically 0)
    # payload[1..3] = 24 conformance bits, MSB-first
    bits = int.from_bytes(payload[1:4], "big")

    supported = {
        name
        for bit, name in _CONFORMANCE_BITS.items()
        if bits & (1 << (23 - bit))
    }
    return Capabilities(raw_conformance=bytes(data), supported=supported)


class CosemException(Exception):
    """
    Raised when a meter returns an exception-response PDU (tag 0x0E).

    This typically means the meter understood the frame but rejected the
    request — e.g. the service isn't supported, or referencing style
    doesn't match what the meter speaks.
    """

    # State-error codes (DLMS Green Book table 6 / 12.7)
    STATE_ERROR_NAMES = {
    ...
    SERVICE_ERROR_NAMES = {
        ...
        4: "pdu-too-long",
        5: "deciphering-error",
        6: "invocation-counter-error",
    }

    def __init__(self, state_error: int, service_error: int):
        self.state_error   = state_error
        self.service_error = service_error
        state_name   = self.STATE_ERROR_NAMES.get(state_error, f"unknown({state_error})")
        service_name = self.SERVICE_ERROR_NAMES.get(service_error, f"unknown({service_error})")
        super().__init__(
            f"COSEM exception: state-error={state_name}, "
            f"service-error={service_name}"
        )


def parse_exception_response(data: bytes) -> CosemException:
    """
    Parse a COSEM exception-response PDU.

        Layout: 0E SE SVE
            0E   exception-response tag
            SE   state-error (1 byte)
            SVE  service-error (1 byte)
    """
    if len(data) < 3 or data[0] != 0x0E:
        raise ValueError(f"Not an exception-response: {data.hex()}")
    return CosemException(state_error=data[1], service_error=data[2])
...
def parse_get_response(data: bytes) -> GetResponse:
    ...
    Raises:
        CosemException: if the meter returned an exception-response (0x0E)
                        instead of a GET.response.
        ValueError:     if the PDU is malformed or an unknown type.
    """
    if len(data) < 1:
        raise ValueError(f"Response too short: {data.hex()}")

    # Exception responses can come back instead of a real GET.response
    if data[0] == 0x0E:
        raise parse_exception_response(data)
